...
.search-input {
  padding:7px 14px 7px 32px; border-radius:99px; border:1.5px solid var(--gris-borde);
  font-family:'DM Sans',sans-serif; font-size:13px; outline:none; width:200px; transition:border-color .15s;
...
.chip-inscrita       { background:#e3f2fd; color:var(--azul); }
.chip-sin_cupos      { background:#ffebee; color:var(--rojo); }
.chip-cruce_horario  { background:#fff3e0; color:#bf360c; }
.chip-limite_creditos{ background:#f3e5f5; color:var(--morado); }
...
.horario-chip { background:#f5f5f5; border-radius:6px; font-size:11px; font-family:'DM Mono',monospace; padding:3px 8px; color:var(--texto-sub); }
...
function cuposHTML(m) {
  const r=m.cupos_restantes, t=m.cupos_total, pct=t>0?Math.round((r/t)*100):0;
  const cls=r===0?'cero':r<=5?'bajo':'ok';
  const txt=r===0?'🔴 Sin cupos':r<=5?`⚠️ Últimos ${r} de ${t}`:`✅ ${r} / ${t} disponibles`;
  return `<div class="cupos-wrap"><div class="cupos-header"><span class="lbl-${cls}">${txt}</span><span style="color:#bbb">${pct}%</span></div><div class="cupos-bg"><div class="cupos-fill ${cls}" style="width:${pct}%"></div></div></div>`;
}
...
function cardHTML(m) {
  ...
  return `<div class="card ${m.estado_ux}" id="card-${m.id}">
    <div class="card-top"><div><div class="card-nombre">${m.nombre}</div><div class="card-codigo">${m.codigo}</div></div><div class="badge-cred">${m.creditos} cr.</div></div>
...
function renderGrid() {
  ...
  document.getElementById('grid').innerHTML = filtradas.length ? filtradas.map(cardHTML).join('') : '<div class="empty">🔍 No se encontraron materias.</div>';
}
async function cargar() {
  document.getElementById('grid').innerHTML = '<div class="loading"><div class="spinner"></div>Cargando materias...</div>';
  try {
    const r = await fetch(API + '/materias.php?estudiante_id=' + Auth.id);
    ...
    document.getElementById('grid').innerHTML = '<div class="empty">⚠️ No se pudo conectar. Verifica que el servidor esté activo.</div>';
  }
}
async function inscribir(materia_id) {
  ...
  try {
    const r = await fetch(API + '/inscribir.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({estudiante_id:parseInt(Auth.id), materia_id}) });
    const data = await r.json();
    if (data.success) { toast(data.mensaje,'ok'); await cargar(); }
    else { toast(data.error,'err'); if (btn) { btn.disabled=false; btn.textContent='+ Inscribir materia'; } }
  } catch(e) { toast('Error de red.','err'); if (btn) { btn.disabled=false; btn.textContent='+ Inscribir materia'; } }
}
cargar();
</script>
</body>
