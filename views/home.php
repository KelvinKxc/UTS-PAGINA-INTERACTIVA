...
<style>
.bienvenida{background:linear-gradient(135deg,var(--verde) 0%,var(--verde-dark) 100%);border-radius:18px;padding:28px 32px;color:white;display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;box-shadow:0 6px 24px rgba(74,124,37,.3)}
...
.badge{background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.2);border-radius:99px;padding:6px 14px;font-size:12px;font-weight:500}
.promedio-circulo{width:88px;height:88px;background:rgba(255,255,255,.15);border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,.3);flex-shrink:0}
...
.acceso-card{background:white;border-radius:14px;padding:22px;box-shadow:var(--sombra);text-decoration:none;color:var(--texto);display:flex;align-items:center;gap:16px;transition:transform .15s,box-shadow .15s;border:1.5px solid transparent}
...
<body>
<?php $pagina_actual='home'; require_once __DIR__ . '/partials/nav.php'; ?>
...
<div class="page">
  ...
  <div class="accesos">
    <a href="<?= BASE_URL ?>/views/inscripcion.php" class="acceso-card">
...
fetch(API + '/resumen.php?estudiante_id=' + Auth.id)
  .then(r => r.json())
  .then(data => {
    if (!data.success) { document.getElementById('stats-grid').innerHTML='<div class="error-api">'+data.error+'</div>'; return; }
    const cr = data.resumen_creditos;
    document.getElementById('promedio-num').textContent   = data.estudiante.promedio;
    document.getElementById('badge-semestre').textContent = 'Semestre ' + data.estudiante.semestre;
    document.getElementById('stats-grid').innerHTML = `
      ...
      <div class="stat-card"><div class="stat-num">${data.estudiante.promedio}</div><div class="stat-label">Promedio acumulado</div></div>`;
    const lista = document.getElementById('mat-lista');
    if (!data.materias_inscritas.length) { lista.innerHTML='<div class="empty-msg">Sin materias inscritas. <a href="<?= BASE_URL ?>/views/inscripcion.php">Ir a inscripción →</a></div>'; return; }
    lista.innerHTML = '<div class="mat-lista">' + data.materias_inscritas.map(m => {
  ...
  .catch(() => { document.getElementById('stats-grid').innerHTML='<div class="error-api">No se pudo conectar al servidor.</div>'; });
</script>
</body>
</html>
